/**
 * @file main.c
 * @brief Main entry point and game loop for Sticky Paws
 *
 * A small game where you control a bear's paw trying to steal
 * honey from a picnic while avoiding waking up the old man watching TV.
 */
#include <raylib.h>
#include <raymath.h>
#include <stdbool.h>
...
static void handlePlayState(GameContext *ctx, UserInterface *ui, Bear *paw, Target *jar,
                            ObstacleArray *obs);
...
static void renderCurrentState(GameContext *ctx, UserInterface *ui, Bear *paw, Target *jar,
                               ObstacleArray *obs)
{
  if (ctx->state == START) {
...
int main()
{
  ...
  Bear Paw = {.tex = LoadTexture("src/assets/sticky_paw.png"),
  ...
  UserInterface GameUI = {.fade = {1.0f, true},
                          .infoBox = {0, 0, 400, 100},
  ...
  while (!WindowShouldClose()) {
    ...
    switch (ctx.state) {
    case START:
      handleStartState(&ctx, &GameUI, &Jar, &Obs);
      break;
    case PLAY:
      handlePlayState(&ctx, &GameUI, &Paw, &Jar, &Obs);
      break;
    case FAIL:
      handleFailState(&ctx, &GameUI, &Jar, &Obs);
      break;
    case WIN:
      handleWinState(&ctx, &GameUI, &Jar, &Obs);
      break;
    }
